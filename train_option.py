...
class Trainer():
    def __init__(
        ...
        self.ds = getattr(datasets, dataset_name)(**dataset_kwargs)
    ...
    def train(self):
        ...
        acc_avg = tm.Accuracy(task='multiclass', num_classes=self.ds.options_n).to('cuda')
        if self.epochs < 0:
...
if __name__ == '__main__':
    with open('configs/config_option.json') as f:
        config = json.load(f)
